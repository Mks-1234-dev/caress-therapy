...
export default function ProfileView() {
  ...
  const [latestQuizResult, setLatestQuizResult] = useState<QuizResult | null>(
    null
  );
  ...
  return (
    <>
      ...
      <div className={styles.card}>
        <div className={styles.title}>Past Moods</div>
          ...
          <div>
            ...
            <div className={styles.row}>
              <div
                ...
                style={{ fontSize: "18px", fontWeight: "420", color: "black" }}
              >
                {mood.date}
              </div>
              <div className={styles.emoji}>{mood.mood}</div>
            </div>
            {/*<br />*/}
          </div>
        ))}
      ...
      {latestQuizResult && (
        <div className={styles.card}>
          <div className={styles.columns}>
            <div className={styles.title}>Personality Traits</div>
            <div className={styles.scores}>
              <ul className={styles.my_list}>
                <li className={styles.li}>
                  Openness: {latestQuizResult.openness}
                </li>
                <li className={styles.li}>
                  Conscientiousness: {latestQuizResult.conscientiousness}
                </li>
                <li className={styles.li}>
                  Extraversion: {latestQuizResult.extraversion}
                </li>
                <li className={styles.li}>
                  Agreeableness: {latestQuizResult.agreeableness}
                </li>
                <li className={styles.li}>
                  Neuroticism: {latestQuizResult.neuroticism}
                </li>
              </ul>
            </div>
            <br />
            <div>1) {latestQuizResult.traits[0]}</div>
            <br />
            <div>2) {latestQuizResult.traits[1]}</div>
            <br />
            <div>3) {latestQuizResult.traits[2]}</div>
            <br />
            <div>4) {latestQuizResult.traits[3]}</div>
            <br />
            <div>5) {latestQuizResult.traits[4]}</div>
            <br />
          </div>
        </div>
      )}
