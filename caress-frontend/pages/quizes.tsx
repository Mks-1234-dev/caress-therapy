...
export default function Quiz() {
      ...
      if (User && !latestResult) {
        ...
        if (!snapshot.empty) {
          ...
          const diff = Math.floor(
            (now.getTime() - latestResultDate.getTime()) / (1000 * 3600 * 24)
          );
  ...
  useEffect(() => {
    async function OfetchLatestResult() {
      ...
      if (user && !OlatestResult) {
        const OlatestResultRef = firebase
          .firestore()
          .collection("users")
          .doc(user.uid)
  ...
  return (
    <div className={styles.content}>
      ...
      <div className={styles.quiz_title}>Different Health Models</div>
      ...
      <div className={styles.container}>
        ...
        <div>
          CARESS model is made by us to be used in therapy to assess and address
          a client's mental and emotional state.{" "}
        ...
        <div>
          {latestResult && daysDiff !== null && daysDiff < 7 ? (
            <button className={styles.btn}>
              Wait {7 - daysDiff} days to take the quiz again
            </button>
          ) : (
            <button
              className={styles.btn}
              onClick={() => router.push("/caress-quiz")}
            >
              Take Quiz
            </button>
          )}
      ...
      <div className={styles.container}>
        <div className={styles.title}>Ocean Model (Personality Test)</div>
        <div>
          The OCEAN model is widely used in psychology research, career
          counseling, and personal development. It can help individuals gain a
          better understanding of themselves and others, and identify areas for
        ...
        {OlatestResult !== null ? (
          <button
            className={styles.btn}
            onClick={() => router.push("/ocean-quiz")}
          >
            Pay 5$ to retake the quiz
          </button>
        ) : (
          <button
            className={styles.btn}
